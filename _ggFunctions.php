...
function ggPin1Count() {
  global $user;
  $ret = ggFetchValue("select count(id) as ctr from tblpin where managerid=$user->id and status='N'");
  return $ret;
}

function ggPin2Count() {
  global $user;
  $ret = ggFetchValue("select count(id) as ctr from tblpin2 where managerid=$user->id and status='N'");
  return $ret;
}
